...
rf_rfe_ctrl <- rfeControl(functions = lmFuncs,  #lmFuncs , rfFuncs
                          method = "repeatedcv",
                          number = 3,
                          repeats = 2,
                          verbose = TRUE,
...
x_rfe <- train_prep %>% select(rf_fs[["variables"]][["var"]][1:50])
...
models <- list(
  ...
  pls = caretModelSpec(method = "pls", tuneLength = 5),
  svmLinear = caretModelSpec(method = "svmLinear", tuneLength = 5),
